...
const AdminInquiries: React.FC = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-12 p-8 pb-40 md:p-16">
      <header className="flex flex-col items-start justify-between gap-8 border-b border-gray-200 pb-12 md:flex-row md:items-end">
      ...
      <div className="space-y-4">
        {filteredInquiries.length === 0 ? (
          <div className="rounded-[4rem] border-2 border-dashed border-gray-200 bg-white p-32 text-center">
            <p className="font-display text-3xl text-gray-500 italic">
          ...
          <div className="space-y-3">
            ...
            {filteredInquiries.map((i) => {
              ...
              return (
                <div
                  key={i.id}
                  className={`group overflow-hidden rounded-[1.5rem] border border-gray-200 bg-white transition-all duration-700 md:rounded-[2.5rem] ${isExpanded ? "scale-[1.01] shadow-2xl ring-2 ring-refenti-gold/10" : "hover:border-gray-300 hover:shadow-lg"} `}
                >
                  {/* Summary Line Row */}
                  <div
                    onClick={() => setExpandedId(isExpanded ? null : i.id)}
                    className="flex cursor-pointer flex-col items-center gap-4 px-12 py-7 text-center lg:grid lg:grid-cols-12 lg:flex-row lg:text-left"
                  >
                    ...
                    <div className="space-y-12 p-10 md:p-16">
                      <div className="flex flex-col items-start justify-between gap-8 md:flex-row">
                        ...
                        <button
                          ...
                          className="flex items-center gap-3 rounded-2xl border border-red-200 bg-white px-10 py-4 text-[10px] font-bold tracking-ultra text-red-500 uppercase shadow-sm transition-all hover:bg-red-500 hover:text-white"
                        >
                          <svg
                            className="h-4 w-4"
                            fill="none"
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="relative overflow-hidden rounded-[4rem] border border-gray-100 bg-white p-12 shadow-inner md:p-20">
                          <span className="absolute top-8 left-10 font-display text-[12rem] text-refenti-gold opacity-5 select-none">
                            “
                          </span>
                          <p className="relative z-10 px-4 font-display text-2xl leading-relaxed font-light text-gray-700 italic md:px-8 md:text-4xl">
                            {i.message}
                          </p>
                          <span className="absolute -right-10 -bottom-16 font-display text-[12rem] text-refenti-gold opacity-5 select-none">
                            ”
                          </span>
                        </div>
                      </div>
